...
- (UIView *) mkContentView
{
    ...
    for (KxMenuItem *menuItem in _menuItems) {
        ...
        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + kMarginY * 2 + 10.0;
    ...
    for (KxMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.enabled) {
            ...
            //[button setBackgroundImage:[imageName(@"MoreFunctionFrameHL") stretchableImageWithLeftCapWidth:20 topCapHeight:10] forState:UIControlStateHighlighted];
              ...
              menuItems:(NSArray *)menuItems
{
    ...
    _menuView.backgroundColor = viewBgColor;
...
+ (BOOL)isMenuOnShow {
    ...
    return [[self sharedMenu] isOnShow];
}

+ (void)setBackGroundColor:(UIColor *)bgColor {

    if (viewBgColor != bgColor) {
        viewBgColor = bgColor;
    }
}
